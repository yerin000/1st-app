...
# -----------------------------------------------------------------------------
# MENU 1: 삼각함수 시작하기 (회전목마 스토리텔링)
# -----------------------------------------------------------------------------
if menu == "1. 삼각함수 시작하기 🎠":
    st.markdown("<div class='main-header'>🎠 삼각함수, 도대체 왜 배울까?</div>", unsafe_allow_dict=True)
    st.markdown("<div class='sub-header'>일상 속의 '반복되는 회전 운동'에서 시작하는 삼각함수 개념</div>", unsafe_allow_dict=True)
...
# -----------------------------------------------------------------------------
    ...
    st.markdown("<div class='main-header'>📖 삼각함수 기초 개념 Master</div>", unsafe_allow_dict=True)
...
# -----------------------------------------------------------------------------
# MENU 3: 핵심 기능 - 삼각함수 그래프 실험실
# -----------------------------------------------------------------------------
elif menu == "3. 그래프 실험실 🔬":
    st.markdown("<div class='main-header'>🔬 삼각함수 그래프 실험실</div>", unsafe_allow_dict=True)
    st.markdown("<div class='sub-header'>$y = A \\sin(Bx + C) + D$ 변수 계수를 직접 움직이며 그래프의 변화를 관찰하세요.</div>", unsafe_allow_dict=True)
    ...
    # A 분석
    if abs(A) != 1.0:
        analysis_text.append(f"• **진폭(A={A})**: 그래프의 최댓값과 최솟값의 폭이 **{abs(A)}배** 변했습니다. (A < 0이면 위아래가 뒤집힙니다.)")
    # B 분석
    period = (2 * np.pi / abs(B)) if "tan" not in func_type else (np.pi / abs(B))
    period_str = f"{period/np.pi:.2f}π"
    analysis_text.append(f"• **주기(B={B})**: 원래 주기에서 변형되어, 한 번 파동이 반복되는 **주기가 [{period_str}]**로 변했습니다. (B가 커질수록 주기는 짧아져 촘촘해집니다.)")
    # C 분석
    if C_deg != 0:
        shift_x = -C_deg / B
    ...
    st.markdown("---")
    # 7. 단위원과 그래프 시각 연동 시메이션
    st.subheader("🔄 단위원운동 ➔ 사인 그래프 연동 애니메이션")
    st.caption("왼쪽 단위원의 Y축 높이가 오른쪽 그래프로 그대로 이어지는 과정을 확인하세요.")
...
# -----------------------------------------------------------------------------
    ...
    st.markdown("<div class='main-header'>🎯 인터랙티브 그래프 맞추기 퀴즈</div>", unsafe_allow_dict=True)
    ...
    # 정답: A=2.0, B=2.0, D=1.0 (최댓값 2+1=3, 최솟값 -2+1=-1, 주기 2pi/2 = pi)
    target_A, target_B, target_D = 2.0, 2.0, 1.0

    # 제출 및 검증
    if st.button("🚀 정답 확인하기"):
        if user_A == target_A and user_B == target_B and user_D == target_D:
            st.markdown("<div class='success-box'>🎉 <b>정답입니다! 완벽해요!</b><br>진폭 A=2, 주기계수 B=2(주기 π), Y축 이동 D=1을 정확하게 맞추셨습니다!</div>", unsafe_allow_dict=True)
            st.session_state.lab_quiz_cleared = True
        else:
            st.markdown("<div class='error-box'>❌ <b>아직 목표와 조금 다릅니다. 아래 힌트를 확인해 보세요!</b></div>", unsafe_allow_dict=True)
...
# -----------------------------------------------------------------------------
    ...
    st.markdown("<div class='main-header'>📝 삼각함수 단계별 평가 및 문제 풀기</div>", unsafe_allow_dict=True)
    ...
    for p in problems:
        ...
        with col_h1:
            ...
            if lvl >= 1:
                st.markdown(f"<div class='hint-box'><b>힌트 1단계:</b> {p['hint1']}</div>", unsafe_allow_dict=True)
            if lvl >= 2:
                st.markdown(f"<div class='hint-box'><b>힌트 2단계:</b> {p['hint2']}</div>", unsafe_allow_dict=True)
            if lvl >= 3:
                st.markdown(f"<div class='hint-box'><b>힌트 3단계:</b> {p['hint3']}</div>", unsafe_allow_dict=True)
...
# -----------------------------------------------------------------------------
    ...
    st.markdown("<div class='main-header'>📓 나의 오답 & 복습 노트</div>", unsafe_allow_dict=True)
...
# -----------------------------------------------------------------------------
# MENU 7: 삼각함수 도우미 (AI Q&A)
# -----------------------------------------------------------------------------
elif menu == "7. 삼각함수 도우미 🤖":
    st.markdown("<div class='main-header'>🤖 삼각함수 챗봇 도우미</div>", unsafe_allow_dict=True)
